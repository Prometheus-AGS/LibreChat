Looking at the OpenAPI specification I created, **no, the authentication endpoints are not included**. The spec only covers the business data endpoints (patients, instructions, plans, etc.) but is missing the crucial authentication endpoints.

Let me update the OpenAPI spec to include the authentication endpoints:Now the OpenAPI specification **includes the authentication endpoints**! Here's what I added:

## ✅ **Authentication Endpoints Now Included:**

1. **`POST /rest-auth/login/`** - Login to get authentication token
2. **`POST /rest-auth/logout/`** - Logout and invalidate token
3. **`POST /rest-auth/password/change/`** - Change password
4. **`GET /rest-auth/user/`** - Get current user details

## **Key Updates Made:**

1. **Server URL**: Changed from `/api/v1` to root URL to include both auth and API endpoints
2. **All API paths**: Updated to include `/api/v1/` prefix
3. **Authentication schemas**: Added `LoginRequest`, `LoginResponse`, and `CurrentUser` schemas
4. **Security**: Added `security: []` to login endpoint (no auth required for login)
5. **Error responses**: Added 401 Unauthorized response schema
6. **Authentication tag**: New tag group for auth-related endpoints

## **The spec now provides complete authentication flow:**

1. **Login**: `POST /rest-auth/login/` with email/password → get token
2. **Use token**: Include `Authorization: Token <token>` in headers for API calls
3. **API calls**: Access any `/api/v1/*` endpoint with authentication
4. **Logout**: `POST /rest-auth/logout/` to invalidate token

This makes the OpenAPI specification complete and ready for generating client SDKs or API documentation that includes the essential authentication workflow.

## Configuring Django-Style Token Authentication

To use the Brius tool, you will need to configure it with Django-style token authentication. This can be done by adding a new `authentication` section to your tool's configuration file.
...
brius:
  authentication:
    ...
    credentials:
      email: ${BRIUS_EMAIL}
      password: ${BRIUS_PASSWORD}
```

### Credentials

The `email` and `password` fields can be provided as environment variables or as user-specific plugin authentication values.
